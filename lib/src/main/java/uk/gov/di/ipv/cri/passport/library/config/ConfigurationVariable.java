...
public enum ConfigurationVariable {
    SESSION_TTL("/%s/SessionTtl"), // Initial Session via Jar Validator / Datastore item ttl
    AUTH_CODE_EXPIRY_CODE_SECONDS("/%s/AuthCodeExpirySeconds"),
    ACCESS_TOKEN_REQ_JWT_TTL("/%s/AccessTokenReqJwtTtl"), // Token Request JWT
    MAX_JWT_TTL("/%s/MaxJwtTtl"), // Issue Cred VC TTL
    MAX_JWT_TTL_UNIT("/%s/JwtTtlUnit"), // Issue Cred VC TTL
